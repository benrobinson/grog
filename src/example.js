import application from './engine/index';
...
import LevelDrawing from './engine/StageDrawing';
...
import * as Degrees from './engine/util/Degrees';
import LevelCollisions from './engine/StageCollisions';
...
function setupApplication(imageAssetManager) {
  ...
  function addPom(app, x, y, player) {
    ...
    app.entities.addEntity(pom, app.stage.drawingLayers.getLayer('entities'));
  ...
  function stage(app) {
    ...
    const levelDrawing = new StageDrawing()
    ...
    const overlay = new StageDrawing()
    ...
    app.stage.drawingLayers.addLayer('background');
    app.stage.drawingLayers.getLayer('background').addDrawable(levelDrawing);
    ...
    app.stage.drawingLayers.addLayer('entities');

    app.stage.drawingLayers.addLayer('foreground');
    app.stage.drawingLayers.getLayer('foreground').addDrawable(overlay);
    ...
    app.entities.addEntity(entity, app.stage.drawingLayers.getLayer('entities'));
    app.entities.addEntity(chicken, app.stage.drawingLayers.getLayer('entities'));
    ...
    app.events.subscribe(Events.common.ENGINE_INPUT, (dt) => {
      ...
      const rot = Degrees.fromVelocity(entity.vx, entity.vy);
      // console.log('rot', rot, 'rotation', rotation, 'rads', Degrees.toRadians(rot), 'radians', radians);
      spriteGroup.sprites.feet.setRotation(rot);
...
function collisionFunctions(app) {
  ...
  function applyFriction(data) {
    ...
    const current = app.stage.collisions.getTypeFromPixels(x, y);

    let friction;

    switch (current) {
      default:
      case StageCollisions.tileTypes.NONE:
      case StageCollisions.tileTypes.FLOOR:
        friction = 20;
        break;
      case StageCollisions.tileTypes.LIQUID:
        friction = 25;
        break;
      case StageCollisions.tileTypes.SLIP:
        friction = 10;
        break;
      case StageCollisions.tileTypes.ROUGH:
        friction = 23;
        break;
    }
